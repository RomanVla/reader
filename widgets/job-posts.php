...
class r_Job_post_c extends WP_Widget {
	...
	{	if(post_type_exists( 'jobpost' )){
        ...
        $page_url = isset($instance['url']) ? esc_attr($instance['url']) : '';
		...
		?>
		 ...
		 <div class="widget-readers-job-board">
			...
			<?php endwhile; ?>	
			 <div><a href="<?php echo esc_url($page_url); ?>" class="btn btn-fullwd btn-prime"><?php _e('View All Jobs','reader') ?></a></div>
	...
	if(post_type_exists( 'jobpost' )){
        ...
        <p>
			<label for="<?php echo esc_attr($this->get_field_id('url')); ?>"><?php _e('Job Page URL:','reader') ?></label>
